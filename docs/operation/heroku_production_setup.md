...
heroku certs:auto:enable -a k2works-portfolio-prd
heroku certs:auto -a k2works-portfolio-prd
# Wildcard / Cert status が "OK" になるまで数分待機
```
...
> **重要**: Heroku ACM が完了する前に「Full (strict)」を選択すると、526 エラー（Invalid SSL certificate）が発生する。順序は「Heroku ACM 有効化 → ACM 完了確認 → Cloudflare で Full (strict) 設定」。
