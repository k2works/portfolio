...
| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|----|------|
| 1. 検索インデックス対応（robots.txt 環境別、sitemap 検証、MkDocs noindex 手順） | 2 | 3h | [x] |
| 2. ハンバーガーメニュー実装 + E2E 5 シナリオ | 1 | 4h | [x] |
| 3. axe-core via Playwright（WCAG 2.1 A/AA で violations 0） | 1 | 2h | [x] |
| 4. Cloudflare ガイド詳細化（3 重防御）+ リリース計画再校正（3 シナリオ） | - | 2h | [x] |
| **合計** | **4** | **11h** | [x] |

**1 SP あたり**: 約 2.75h
**実績**: 約 2h（IT-1/IT-2 同様に手動構築の効率化で短縮、ハンバーガー JS は若干の試行錯誤あり）
**進捗率**: 100%（4/4 SP）
