...
**実績ベロシティ**: 12 SP / 約 5h = **2.4 SP/h**（IT-1 単独 1.67 → IT-2 単独 ~3.5、上昇傾向）

> 高生産性の理由は「設計ドキュメントが豊富 + 手動構築の効率化 + Codex 不使用 + 個人開発の意思決定速度」。IT-3 以降は外部依存（Heroku/Cloudflare 実機セットアップ）が増えるため低下する想定。
...
| 3（計画済み） | 検索インデックス + ハンバーガー + axe-core + Cloudflare ガイド（IT-3） | 4 | 16 | **v0.1 リリース準備** |
| - | Heroku/Cloudflare 実機セットアップ + main マージ + リリース完了報告書 | -（外部依存） | 16 | **v0.1 リリース** |
| 4 | US-02 一覧 + Content Collections（works コレクション） | 7 | 23 | v0.2-α |
| 5 | US-03 詳細 + 5 件のサンプル Works 投入 + Works フィルタ | 6 | 29 | **v0.2 リリース**（v0.2 = 10 SP） |
| 6 | US-04 Skills + US-07 ダークモード切替 | 7 | 36 | v0.3-α |
| 7 | US-05 + US-06 + US-08 モバイル仕上げ + ダークトグル仕上げ | 6 | 42 | **v0.3 リリース**（v0.3 = 13 SP） |
| 8 | US-10 A11y 強化（NVDA / VoiceOver 手動検証 + axe-core 全画面） | 5 | 47 | v1.0-α |
| 9 | US-11 Tech Notes 同居（noindex / 戻り動線）+ US-12 OGP（@astrojs/og） | 5 | 52 | **v1.0 リリース**（v1.0 = 10 SP） |
...
|---|---|---|---|---|---|
| 楽観 | 7 SP/週（IT-1〜IT-2 ペース継続） | 2026-05-04 | 2026-05-18 | 2026-06-08 | 2026-06-22 |
| 標準 | 5 SP/週（IT-3 以降は実装複雑度↑で減速） | 2026-05-11 | 2026-06-01 | 2026-06-29 | 2026-07-20 |
| 悲観 | 3 SP/週（外部依存で停滞・体調不良など） | 2026-05-18 | 2026-06-29 | 2026-08-31 | 2026-10-19 |

> **外部依存リスク**: ドメイン取得 / Cloudflare DNS 委譲伝播（最長 24h） / Heroku アカウント審査などはコード進捗と独立。v0.1 リリースは「コード完成」と「公開可能」を区別して記録する。
