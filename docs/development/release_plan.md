...
### IT-1〜IT-7 実績（2026-05-01 時点・v0.3 リリース完了）
...
| **累計** | **43** | **43** | **92.6h** | **約 13h** | **100%** |

**実績ベロシティ**: 43 SP / 約 13h = **3.31 SP/h**（IT-1 1.67 → IT-2 3.50 → IT-3 2.00 → IT-4 4.67 → IT-5 3.00 → IT-6 4.67 → IT-7 **7.00**）

> v0.1 / v0.2 / v0.3 全体での生産性は「設計ドキュメントが豊富 + 手動構築の効率化 + Codex 不使用 + 個人開発の意思決定速度」によって達成。IT-7 単独で **7.00 SP/h**（全イテレーション中ピーク、IT-6 までの記録 4.67 SP/h を更新）。pre-commit hook + .gitattributes 拡張で品質ゲートが堅牢化され、IT-7 期間中の手戻りがゼロ。残 IT-8 / IT-9 で v1.0 リリース（残 10 SP: US-10 + US-11 + US-12）を見込む。
...
| 8 | US-10 A11y 強化（キーボード操作網羅 + フォーカストラップ + NVDA / VoiceOver 手動検証 + Lighthouse v1.0 予算 A11y≥95）（IT-8） | 5 | 52 | v1.0-α |
