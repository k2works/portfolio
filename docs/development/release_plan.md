...
### IT-1〜IT-8 実績（2026-05-01 時点・v1.0-α 完了）
...
| IT-7 | 7 | 7 | 13.5h | 約 1h | 完了（v0.3 リリース） |
| IT-8 | 5 | 5 | 8.3h | 約 0.5h | 完了（v1.0-α） |
| **累計** | **48** | **48** | **100.9h** | **約 13.5h** | **100%** |

**実績ベロシティ**: 48 SP / 約 13.5h = **3.56 SP/h**（IT-1 1.67 → IT-2 3.50 → IT-3 2.00 → IT-4 4.67 → IT-5 3.00 → IT-6 4.67 → IT-7 7.00 → IT-8 **10.00**）

> v0.1 / v0.2 / v0.3 / v1.0-α 全体での生産性は「設計ドキュメントが豊富 + 手動構築の効率化 + Codex 不使用 + 個人開発の意思決定速度」によって達成。IT-8 単独で **10.00 SP/h**（全イテレーション中ピーク、IT-7 の記録 7.00 SP/h を更新）。要因は (1) スキップリンク / ランドマーク / focus-visible が v0.1 から準備済みでテスト追加が中心だった、(2) フォーカストラップ実装が 30 行程度の小規模、(3) pre-commit hook + .gitattributes で品質ゲート堅牢化、(4) ホーム Content Collections 連動も Featured 選定基準を IT-5 で明文化済み。残 IT-9 で v1.0 リリース（残 5 SP: US-11 + US-12）を見込む。
