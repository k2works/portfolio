...
### IT-1〜IT-5 実績（2026-05-01 時点・v0.2 リリース完了）
...
| **累計** | **29** | **29** | **65.1h** | **約 10.5h** | **100%** |

**実績ベロシティ**: 29 SP / 約 10.5h = **2.76 SP/h**（IT-1 1.67 → IT-2 3.50 → IT-3 2.00 → IT-4 4.67 → IT-5 3.00）

> v0.1 / v0.2 全体での生産性は「設計ドキュメントが豊富 + 手動構築の効率化 + Codex 不使用 + 個人開発の意思決定速度」によって達成。IT-4 / IT-5 でも設計先行ボーナスが継続して効き、v0.2 平均は **6.5 SP/イテレーション** / **3.71 SP/h**。IT-6 以降の v0.3 では標準 5〜6 SP/週で見積もる（Skills の Markdown 作成 + ダークモード CSS 整理が時間取る想定）。
...
| 6 | US-04 Skills + US-07 ダークモード切替（IT-6） | 7 | 40 | v0.3-α |
| 7 | US-05 + US-06 + US-08 モバイル仕上げ + ダークトグル仕上げ（IT-7） | 6 | 46 | **v0.3 リリース**（v0.3 = 13 SP） |
