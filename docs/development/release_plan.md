...
### IT-1 + IT-2 + IT-3 実績（2026-04-30 時点・v0.1 リリース完了）
...
**実績ベロシティ**: 16 SP / 約 7h = **2.29 SP/h**（IT-1 単独 1.67 → IT-2 単独 3.5 → IT-3 単独 2.0）

> v0.1 全体での生産性は「設計ドキュメントが豊富 + 手動構築の効率化 + Codex 不使用 + 個人開発の意思決定速度」によって達成。IT-3 は外部依存（CI 認証問題で 4 回手戻り）でやや低下したが、計画 SP は完遂。v0.2 以降は標準 5 SP/週で見積もる（設計先行ボーナスが効きにくくなる想定）。
